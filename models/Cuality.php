...
/**
 * This is the model class for table "cuality".
 *
 * @property integer $id
 * @property string $ref
 ...
 */
class Cuality extends \yii\db\ActiveRecord
{
     ...
     if(in_array($type, ['covenant'])){
            ...
            if(is_array($files)){
                 ...
                 foreach ($files as $key => $value) {
                    $docs_file .= '<li>'.Html::a($value,['/cuality/download','id'=>$this->id,'file'=>$key,'file_name'=>$value]).'</li>';
                 }
                 $docs_file .='</ul>';
            }
     }
                 ...
                 foreach ($files as $key => $value) {
                    if($type=='file'){
                        ...
                        $initial[] = [
                            'caption'=> $value,
                            'width'  => '120px',
                            'url'    => Url::to(['/cuality/deletefile','id'=>$this->id,'fileName'=>$key,'field'=>$field]),
                            'key'    => $key
                        ];
                    }
                    else{
